...
helper.create_boxplot <- function(data, x_var, y_var = NULL, fill_color = "blue", title_prefix = "Box Plot of ") {
  if (!is.null(y_var)) {
...
helper.analyze_variable <- function(data, var_name, dependent_var = "Recent.Close.Price") {
  # histogram
  p1 <- helper.plot_histograms(data = data, variable_name = var_name)
  ...
  print(p3)
  # Density Plot
  p4 <- ggplot(data, aes_string(x = var_name)) +
...
detect_nonlinearities_and_outliers <- function(data, dependent_var, predictor_var) {
  # nested helper because we dont need elsewhere 
  # perform power transformation and return lambda
  ...
  # first original model then transformed
  ...
  # now we detect outliers based on model 1 and make no outlier model
  ...
  # plot them graphs
  par(mfrow = c(2, 1))  # Set up 2x2 plotting area
  
  # scatterplot for predictor vs transformed
  ...
  abline(h = 4 * mean(outlier_info$cooks_d, na.rm = TRUE), col = "red", lty = 2)
  
  
  # Plot the models themselves (gives four graphs)
  ...
  # this puts all in one graph which makes nice to look at 
...
# wanted to remove EPS and PE from interaction_var so made hlper function
helper.remove_specific_rows <- function(data, var1_pattern, var2_pattern) {
  # Create logical index for rows to keep s/o regex and eggert
...
I have decided to omit all rows that posess null values because I do not think there is a fair way to fill them in and we are still left with more than enough variables to continue our analysis. This serves as the answer to how I deal with null values. 

# 1 Variable Selection

### Boruta Algorithm Selection
```{r Boruta Selection, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
set.seed(123)
...
boruta_result_stock <- Boruta(Recent.Close.Price ~ ., data = stock_data, doTrace = 2, verbose = FALSE)
...
```{r EDA for factor variables, warning=FALSE}
...
We plot a histogram, qq-plot, density plot, and box plot. We see that it is right skewed and not very normally distributed suggesting that a transformation may be ideal to make it more normal. We also see the presence of outlier especially on the higher side of the boxplot.

```{r outlier and linearity detection for EPS, warning = FALSE, message= FALSE}
...
results$influential_outliers 

```

In my opinion for EPS there is no reason to do a powerTransform as the R^2 values after the powerTransform actually decreases and although it definitely makes it more normally distributed, I do not think it is statistically significant in the premise of making a multiple linear regression model. The next question then comes to outliers. We used Cook's Distance to show that we have 10 outliers in our model that we identified that have a standard deviation of greater than 4 then our mean Cook's Distance. I do not think it is worth removing these outliers as the R^2 does not change drastically with their exclusion.
...
```{r outlier and linearity detection for PB Ratio, warning= FALSE, message= FALSE}
results <- detect_nonlinearities_and_outliers(
  data = stock_data, 
  dependent_var = "Recent.Close.Price", 
...
```{r outlier and linearity detection for PE Ratio, warning= FALSE, message= FALSE}
results <- detect_nonlinearities_and_outliers(
  data = stock_data, 
  dependent_var = "Recent.Close.Price", 
...
We plot a histogram, qq-plot, density plot, and box plot. We see that it is right skewed and not very normally distributed suggesting that a transformation may be ideal to make it more normal. We also see the presence of outlier especially on the higher side of the boxplot. The QQ plot is better for this one but it too tails off at the higher extremes.

```{r outlier and linearity detection for Avg Volume, warning=FALSE, message= FALSE}
data = stock_data
dependent_var = "Recent.Close.Price"
...
```
We plot a histogram, qq-plot, density plot, and box plot. We see that it is right skewed and not very normally distributed suggesting that a transformation may be ideal to make it more normal. We also see the presence of outliers especially on the higher side of the boxplot.

```{r outlier and linearity detection for Net Income, warning= FALSE, message= FALSE}
data = stock_data 
dependent_var = "Recent.Close.Price"
...
summary(model_3)
```
So far every single one of these variables are statistically significant so as a result we will proceed with this model for further analysis. Moreover we will be testing these variables with the Ramsey RESET test for model misspecification and have omitted the two other variables from future consideration and testing.
...
for (var in quantitative_vars) {
  ...
  print("") # idk how to do a new line in this godforsaken language 
}
```
...
### Testing possible Permutations to find best model through BIC and adjusted R^2
...
test_interaction_terms <- function(data, dependent_var, selected_vars) {
  ...
  # s/o pairs once again
  pairs <- combn(selected_vars, 2, simplify = FALSE)
  
  for (pair in pairs) {
    # i was getting duplicate this eliminates it (still computationally long but too lazy to figure out optimization)
    pair <- sort(pair)
    var1 <- pair[1]
    var2 <- pair[2]
    ...
    # R^2, AIC, and BIC for interaction model
...
The residual plot looks good, and there do not appear to be a concerning amout of outliers.
...
```
From this I am leaning towards `model_5` heavily as it has the lowest AIC.

### Cross Validation
```{r cross validation}
# ok so there was never any mention of cross validation in any part of class so I just used what I have learned in previous stat classes for cross validation. If this is wrong I feel like it should have been clarified in class. 
...
n_boot <- 1000
n_coef <- length(coef(model_5))
coef_names <- names(coef(model_5))

# Create storage matrices
coef_matrix <- matrix(0, nrow = n_boot, ncol = n_coef)
colnames(coef_matrix) <- coef_names
ci_widths <- matrix(0, nrow = n_boot, ncol = n_coef)
colnames(ci_widths) <- coef_names
bootstrap_r2 <- numeric(n_boot)
...
for (i in 1:n_boot) {
  ...
  coef_matrix[i,] <- coef(reg_result)
  
  # Store CI widths for all coefficients
...
for(j in 1:n_coef) {
  hist(coef_matrix[,j], 
       main = paste("Bootstrap Slope", coef_names[j]), 
       xlab = "Coefficient",
       col = "lightblue",
       breaks = 30)
}

# Plot CI width distributions
for(j in 1:n_coef) {
  hist(ci_widths[,j],
       main = paste("CI Width -", coef_names[j]),
       xlab = "CI Width",
       col = "lightpink",
       breaks = 30)
}
...
The bootstrapping results show stable estimates for the intercept, slopes, and R^2, with narrow confidence intervals for both the slope and intercept. The model appears well-specified, with no major patterns in the residuals. With this I will conclude testing for our model and proceed to the Summary. The two interesting ones were the `P.E.Ratio` polynomial terms which were heavily skewed and not at all normally distributed, but it appears that they have very small marginal effects and as a result we will ignore them. 
